...
class JobService {
  ...
  Future<List<ScanItemModel>> getMySharedImages() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('token');
      ...
      final response = await _dio.get(
        ApiConstants.getMySharedImagesEndpoint, 
        options: Options(
          headers: {
      ...
      if (response.statusCode == 200) {
        ...
        return imagesData.map((json) => ScanItemModel.fromJson(json)).toList();
      } else {
        throw Exception('Failed to load shared images');
      }
    } on DioException catch (e) {
      throw Exception(e.response?.data['detail'] ?? "Error fetching images");
    }
  }
}